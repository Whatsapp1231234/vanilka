...
export async function participantsUpdate({ id, participants, action }) {
    ...
    switch (action) {
        ...
        case 'remove':
            ...
            break
        case 'promote':
            text = (chat.sPromote || this.spromote || conn.spromote || '@user Назначен админом группы')
        case 'demote':
            let pp = await this.profilePictureUrl(participants[0], 'image').catch(_ => 'https://i.imgur.com/whjlJSf.jpg') 
            if (!text)
                text = (chat.sDemote || this.sdemote || conn.sdemote || '@user больше здесь не начальник!')
...
≡ Вы удалили свое сообщение в группе!Предупреждаю!  
┌─⊷  Антиудаление
...
global.dfail = (type, m, conn) => {
    let msg = {
        ...
        owner: '🔱 Эта команда может использоваться только *Создателем бота*',
        mods: '🔰  Эта функция предназначена только для *админов*',
        premium: '💠 Эта команда предназначена только для участников *Премиум*\n\nВведите * / premium* для получения дополнительной информации',
        group: '⚙️ ¡Эту команду можно использовать только в группах!',
        private: '📮 Эту команду можно использовать только в чате *частный бот*',
