...
export async function participantsUpdate({ id, participants, action }) {
    ...
    switch (action) {
        ...
        case 'remove':
                ...
                for (let user of participants) {
                    ...
                    try {
                        ...
                        text = (action === 'add' ? (chat.sWelcome || this.welcome || conn.welcome || 'Bienvenido, @user').replace('@group', await this.getName(id)).replace('@desc', groupMetadata.desc?.toString() || 'Desconocido') :
                            (chat.sBye || this.bye || conn.bye || 'До встречи, @user')).replace('@user', '@' + user.split('@')[0])
            ...
            break
        case 'promote':
            text = (chat.sPromote || this.spromote || conn.spromote || '@user теперь с привелегиями админа!)))')
        case 'demote':
            let pp = await this.profilePictureUrl(participants[0], 'image').catch(_ => 'https://i.imgur.com/whjlJSf.jpg') 
            if (!text)
                text = (chat.sDemote || this.sdemote || conn.sdemote || '@user здесь больше не хозяин!)))')
...
≡ Предупреждаю что вы удалили свое сообщение 
┌─⊷  Антиудаление 
